...
int main() {
    ...
    while (fgets(buffer, 15, fp)) {
        // check only once, all numbers have the same amount of bits
        // -1 because of newline
        if (size == 0) size = strlen(buffer) - 1;

        // increments the bit counters
        for (int i = 0; i < size; i++) {
            if (buffer[i] == '0') c0[i]++;
            else if (buffer[i] == '1') c1[i]++;
        }
    }

    // create the gamma int
    for (int i = 0; i < size; i++) {
        // multiply by 2
        gamma <<= 1;
        // append 0b1
    ...
    // invert gamma, using the number of bits
    int mask = (1 << size) - 1;
    epsilon = ~gamma & mask;
