...
// kep track if a small cave is visited twice
int vis_twice = 0;
...
void count_paths(int n, int mat[][n], int src) {
    ...
    // if on start and start already in visited caves
    if (strcmp(name_index[src], "start") == 0) {
        for (int i = 0; i < cave_top; i++) {
    ...
    // if visited 2 smol caves now, reset
    if (counter) vis_twice = 0;
